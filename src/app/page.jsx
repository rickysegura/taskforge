  ...
  const features = [
    {
      ...
      icon: (<><rect x="10" y="10" width="28" height="28" rx="4" fill="none" stroke="#2dd4bf" strokeWidth="2"/><path d="M18 24 L22 28 L30 20" stroke="#2dd4bf" strokeWidth="3" strokeLinecap="round"/></>)
    },
    {
      title: "Secure Authentication",
      desc: "Sign in securely with email or Google, keeping your tasks safe.",
      icon: (<><rect x="14" y="20" width="20" height="16" rx="2" fill="none" stroke="#2dd4bf" strokeWidth="2"/><path d="M18 20 V16 A6 6 0 0 1 30 16 V20" stroke="#2dd4bf" strokeWidth="2" strokeLinecap="round"/><circle cx="24" cy="28" r="2" fill="#2dd4bf"/></>)
    },
    {
      title: "Completely Free",
      desc: "Enjoy all TaskForge features at no cost!",
      icon: (<><circle cx="24" cy="24" r="14" fill="none" stroke="#2dd4bf" strokeWidth="2"/><path d="M24 18 V30 M18 24 H30" stroke="#2dd4bf" strokeWidth="2" strokeLinecap="round"/></>)
    }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-100 via-blue-200 to-purple-200 text-gray-800">
      ...
      <header className="flex justify-between items-center p-6 backdrop-blur-md bg-white bg-opacity-80">
        ...
        <div className="flex gap-4 items-center">
          <a href={`https://x.com/${xHandle.slice(1)}`} target="_blank" rel="noopener noreferrer" className="text-teal-600 hover:text-teal-700 transition-colors duration-300">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="hover:scale-110 transition-transform duration-300">
      ...
      <section className="flex flex-col md:flex-row items-center justify-between p-10 mx-6 mt-6">
        ...
        <div className="mt-10 md:mt-0 md:ml-10 transform hover:scale-105 transition-transform duration-300 animate-fadeIn" style={{ animationDelay: "0.5s" }}>
          <img src="/application.png" alt="TaskForge Desktop Screenshot" className="w-full h-full shadow-lg rounded-lg object-cover"/>
        </div>
      </section>
      ...
      <section className="p-10 rounded-2xl mx-6 shadow-xl backdrop-blur-md bg-white bg-opacity-80">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {features.map((feature, index) => (
            <div key={index} className="p-6 rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-md bg-teal-50 bg-opacity-70 text-gray-800 group animate-fadeIn" style={{ animationDelay: `${index * 0.2}s` }}>
              <svg className="mb-4 transform transition-transform duration-300 group-hover:scale-110" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">{feature.icon}</svg>
